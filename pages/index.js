...
const Cell = ({ col, c, onUpdate }) => {
  ...
  if (col.editable) return (
    <input value={val} onChange={e => onUpdate(c._id, col.key, e.target.value)}
      placeholder="rTR..."
...
export default function App() {
  ...
  const persist         = (list)           => { setCandidates(list); try { localStorage.setItem(STORAGE_KEY, JSON.stringify(list)); } catch {} };
  const addCandidate    = (r)              => persist([...candidates, { _id:Date.now(), candidate_id:"", ...r }]);
  const removeCandidate = (id)             => persist(candidates.filter(c => c._id !== id));
  const updateField     = (id, key, val)   => persist(candidates.map(c => c._id===id ? {...c,[key]:val} : c));
  const clearAll        = ()               => { persist([]); setConfirmClear(false); };
            ...
            <>
              ...
              {mode==="paste" ? (
                ...
                <div onClick={() => fileRef.current.click()}
                  style={{ border:"2px dashed #ccc", borderRadius:6, padding:"32px 24px", textAlign:"center", cursor:"pointer", background:"#fafafa" }}>
                  <div style={{ fontSize:13, color:"#555", marginBottom:4 }}>{fileName?`✓ ${fileName}`:"Click to upload a PDF CV"}</div>
                  {!fileName && <div style={{ fontSize:11, color:"#aaa" }}>PDF files only</div>}
              ...
              <button onClick={extract} disabled={loading}
                ...
                {loading?"Extracting...":"Extract candidate data"}
              </button>
            </>
          ) : (
            <>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ width:38, height:38, borderRadius:"50%", background:RED, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:"bold", fontSize:13, flexShrink:0 }}>
          ...
          {candidates.length===0 ? (
            ...
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:10 }}>
                <div style={{ fontSize:13, color:"#555" }}>{candidates.length} candidate{candidates.length!==1?"s":""} · scroll right to see all fields</div>
              ...
              <div style={{ overflowX:"auto", border:"1px solid #e0e0e0", borderRadius:6 }}>
                <table style={{ borderCollapse:"collapse", fontSize:11, width:TABLE_W, tableLayout:"fixed" }}>
                  ...
                  <tbody>
                    {candidates.map((c,i) => (
                      <tr key={c._id} style={{ background:i%2===0?"#fafafa":"#fff", borderBottom:"1px solid #eee" }}>
                        {ALL_COLS.map(col => (
                        ...
                        <td style={{ padding:"4px 6px", textAlign:"center" }}>
                          <button onClick={() => removeCandidate(c._id)} title="Remove"
              ...
              <div style={{ marginTop:10, fontSize:11, color:"#aaa" }}>
                ID column is editable. Hover any cell for full text. Excel download includes all 26 fields.
              </div>
            </>
          )}
        </div>
